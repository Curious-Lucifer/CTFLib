...
class BigramAnalysis:
    def __init__(self):
        ...
        self.N = sum(self.bigrams.values())
        self.floor = log10(0.01 / self.N)

        for key in self.bigrams.keys():
            self.bigrams[key] = log10(self.bigrams[key] / self.N)
    ...
    def score(self, msg: str):
        ...
        for i in range(len(msg) - 1):
            if msg[i: i + 2] in self.bigrams:
                score += self.bigrams[msg[i: i + 2]]
            else:
                score += self.floor
        return score
...
def playfair_decrypt(cipher: str, key: str):
    """
    - input : `cipher (str)`, `key (str)`
    - output : `plain (str)`
    """

    plain = ''
    for i in range(0, len(cipher), 2):
        row1, col1 = key.index(cipher[i]) // 5, key.index(cipher[i]) % 5
        row2, col2 = key.index(cipher[i + 1]) // 5, key.index(cipher[i + 1]) % 5
        if row1 == row2:
            plain += key[row1 * 5 + (col1 - 1) % 5] + key[row1 * 5 + (col2 - 1) % 5]
        elif col1 == col2:
            plain += key[((row1 - 1) % 5) * 5 + col1] + key[((row2 - 1) % 5) * 5 + col1]
        else:
            plain += key[row1 * 5 + col2] + key[row2 * 5 + col1]
    return plain
...
def crack_vigenere(cipher: str, charset: str):
    ...
    bigram_analysis = BigramAnalysis()
    ...
    val_list = [bigram_analysis.score(plain) for plain in plain_list]
